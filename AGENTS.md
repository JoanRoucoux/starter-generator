...
- Manifest application order is part of the contract: `remove` → `jsonPatch` → `replace` → render templates → copy `--openapi` spec → delete `generator.config.json` + `.generator/`.
...
- `__token__` segments in template **paths** are fatal when unknown (unlike content tokens, which only warn).
